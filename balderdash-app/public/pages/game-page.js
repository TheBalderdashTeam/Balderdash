...
export class GamePage extends HTMLElement {
  ...
  async fetchRoundData() {
    ...
    const fetchRequests = [
      apiFetch('games', { method: 'GET' }),
    ];

    let needsRoundData = false;

    if (!this.roundData) {
      fetchRequests.push(apiFetch('games/current-round', { method: 'GET' }));
      needsRoundData = true;
    }
  ...
  updateSelection() {
    ...
    const selected = this.shadow.querySelector(`#${this.selectedDefinitionId}`);
  ...
  updateContent() {
    ...
    if (Array.isArray(this.roundData?.definitions)) {
      ...
      const allDefinitions = [...this.roundData.definitions, this.roundData.word]
