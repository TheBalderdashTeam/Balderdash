...
resource "aws_db_instance" "balderdashdb" {
  identifier             = "balderdashdb"
  engine                 = "postgres"
  engine_version         = "16.4"
...
output "db_host" {
  value = aws_db_instance.balderdashdb.endpoint
  description = "The endpoint of the Postgres Server RDS instance"
}


resource "aws_security_group" "ec2_security_group" {
  name_prefix = "balderdash_api_sg"
...
resource "aws_instance" "balderdash_ec2_instance" {
  ami           = "ami-0b7e05c6022fc830b"
  instance_type = "t3.micro"
  key_name      = "balderdash-key"
  tags = {
    Name = "balderdash_ec2_instance"
  }
...
resource "aws_eip" "balderdash_ec2_eip" {
  instance = aws_instance.balderdash_ec2_instance.id
  domain   = "vpc"
}

output "ec2_host" {
  value = aws_eip.balderdash_ec2_eip.public_dns
